...
def get_session(gpu_fraction=0.333):
    gpu_options = tf.GPUOptions(per_process_gpu_memory_fraction=gpu_fraction,
                                allow_growth=True)
    return tf.Session(config=tf.ConfigProto(inter_op_parallelism_threads=6,intra_op_parallelism_threads=6,gpu_options=gpu_options))
...
def show_results(img,results, img_width, img_height, model_age, model_gender, model_emotion, time_start, lines_age, lines_gender, lines_fer2013):
	...
	for i in range(len(results)):
		...
		cv2.rectangle(target_image, (xmin2,ymax2), (xmax2,ymax2+80), color=(192,192,192), thickness=-1)
	...
	#cv2.namedWindow('Tongfang mkl-dnn inference Demo', cv2.WINDOW_NORMAL)
	cv2.imshow('Tongfang mkl-dnn inference Demo',img_cp)
...
def main(argv):
	...
	if(os.path.exists(MODEL_ROOT_PATH+'fer2013_mini_XCEPTION.107-0.66.hdf5')):
	...
	#Prepare WebCamera
	#cap = cv2.VideoCapture("F:/soft/waifu2x/ffmpeg-20180224-c0919c4-win64-static/bin/test1.avi")
	...
	while True:
		...
		show_results(img_cv,results, img.shape[1], img.shape[0], model_age, model_gender, model_emotion, time_start, lines_age, lines_gender, lines_fer2013)
