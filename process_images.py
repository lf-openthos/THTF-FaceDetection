...
#display result
def show_results(img,results, img_width, img_height, model_age, model_gender, model_emotion, time_start, lines_age, lines_gender, lines_fer2013, filename):
	img_cp = img.copy()
	target_image=img_cp
...
def main(argv):
	...
	for parent,dirnames,filenames in os.walk("./images"):
		...
		for filename in filenames:
			...
			img_h, img_w, img_depth = frame.shape
			f_scale = 1
			if img_w > 1920: 
				f_scale = img_w/1920
			...
			show_results(img_cv,results, img.shape[1], img.shape[0], model_age, model_gender, model_emotion, time_start, lines_age, lines_gender, lines_fer2013, filename)
